...
export function RepositoriesTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t.settings.repositories.repositoriesSaved);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t.settings.repositories.failedToSaveRepositories);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <section className="space-y-4">
        <h2 className="text-sm font-semibold">{t.settings.repositories.repositories}</h2>

        <Card>
          <CardContent className="space-y-3">
            <p className="text-xs text-muted-foreground">
              {t.settings.repositories.repositoriesDescription}
            </p>
            ...
            {!canManageWorkspace && (
              <p className="text-xs text-muted-foreground">
                {t.settings.repositories.onlyAdminsAndOwnersCanManage}
              </p>
            )}
          </CardContent>
        </Card>

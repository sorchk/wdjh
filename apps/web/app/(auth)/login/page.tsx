...
function LoginPageContent() {
  ...
  useEffect(() => {
    ...
    if (isDesktopHandoff) {
      api
        ...
        .catch((err) => {
          setError(err instanceof Error ? err.message : "Failed to prepare Desktop sign-in");
        });
      return;
    }
    if (!hasOnboarded) {
    ...
    const list = qc.getQueryData<any[]>(workspaceKeys.list()) ?? [];
  ...
  const handleCredentialsLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const onboarded = data.user.onboarded_at != null;
      if (!onboarded) {
        router.push(paths.onboarding());
        return;
      }
      const [first] = wsList;
      router.push(first ? paths.workspace(first.slug).issues() : paths.newWorkspace());
    } catch (err) {
